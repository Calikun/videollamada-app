...
webrtc_rtt_gauge = Gauge('webrtc_rtt_ms', 'Round Trip Time in ms', ['room_id', 'user_id', 'peer_id'])
webrtc_bitrate_gauge = Gauge('webrtc_bitrate_kbps', 'Video Bitrate in kbps', ['room_id', 'user_id', 'peer_id'])
webrtc_packet_loss_gauge = Gauge('webrtc_packet_loss_total', 'Total Packets Lost', ['room_id', 'user_id', 'peer_id'])
webrtc_jitter_gauge = Gauge('webrtc_jitter_ms', 'Jitter in ms', ['room_id', 'user_id', 'peer_id'])
    ...
    try:
        ...
        try:
            ...
            if not admin:
                ...
                # Resetear contraseña del admin para asegurar que funcione
                admin.password_hash = hash_password("admin123")
                admin.is_admin = True
                db.commit()
                logger.info("ℹ️ Admin por defecto ya existe — contraseña reseteada")
...
async def eliminar_usuario(user_id: str, request: Request):
    ...
    try:
        ...
        # Proteger al administrador por defecto
        if usuario.email == "admin@ensigan.corp":
            logger.warning(f"[ADMIN] Intento de eliminar al admin por defecto bloqueado por {admin_name}")
            return JSONResponse(
                status_code=403,
                content={"detail": "No se puede eliminar al administrador por defecto del sistema"}
            )
...
async def cleanup_user(room: Dict, room_id: str, websocket: WebSocket, user_id: str, username: str):
    ...
    try:
        ...
        # Limpiar métricas de Prometheus para este usuario
        for uid in list(room["user_ids"]):
            try:
                webrtc_rtt_gauge.remove(room_id, user_id, uid)
                webrtc_bitrate_gauge.remove(room_id, user_id, uid)
                webrtc_packet_loss_gauge.remove(room_id, user_id, uid)
                webrtc_jitter_gauge.remove(room_id, user_id, uid)
            except KeyError:
                pass
...
async def websocket_endpoint(websocket: WebSocket, room_id: str):
    ...
    try:
        while True:
            ...
            # ── Comando: Estadísticas WebRTC (Prometheus) ──
            if msg_type == "webrtc-stats":
                stats = message.get("stats", {})
                for peer_id, peer_stats in stats.items():
                    webrtc_rtt_gauge.labels(room_id=room_id, user_id=user_id, peer_id=peer_id).set(peer_stats.get('rtt', 0))
                    webrtc_bitrate_gauge.labels(room_id=room_id, user_id=user_id, peer_id=peer_id).set(peer_stats.get('bitrate', 0))
                    webrtc_packet_loss_gauge.labels(room_id=room_id, user_id=user_id, peer_id=peer_id).set(peer_stats.get('packetsLost', 0))
                    webrtc_jitter_gauge.labels(room_id=room_id, user_id=user_id, peer_id=peer_id).set(peer_stats.get('jitter', 0))
                continue
